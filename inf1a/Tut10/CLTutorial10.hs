...
ddelta (FSM  qs as ts ss fs ) source  sym =
  fromList [ to | (from, s, to) <- toList ts
             , from `member` source
             , sym == s ]
...
next fsm@(FSM qs as ts ss fs) supers =
   fromList [ ddelta fsm super sym
            | super <- toList supers, sym <- toList as
            ]
...
dfinal  fsm@(FSM qs as ts ss fs) supers = filterS isFinal supers
  where
    isFinal super = any (\s -> member s fs) super
