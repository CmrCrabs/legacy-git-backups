...
eval vn (p :<->: q) = eval vn p && eval vn q || (eval vn (Not p) && eval vn (Not q))
-- end
...
prop_taut :: Prop -> Bool
--prop_taut p = if tautology p then tautology p == satisfiable (Not (Var "P")) else True
prop_taut p = tautology p == not (satisfiable (Not (Var "P")))
